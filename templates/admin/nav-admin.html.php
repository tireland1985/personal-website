<li class="admin-nav"><a href="/admin/home">Administration</a></li>
<br />

<li class="admin-nav">Portfolio:</li>
<li class="admin-nav"><a href="/portfolio/showPersonal">Personal Projects</a></li>
<li class="admin-nav"><a href="/portfolio/showUniversity">University Projects</a></li>
<li class="admin-nav"><a href="/portfolio/showProjects">All Projects</a></li>

<br />
<li class="admin-nav">CV:</li>
<li class="admin-nav"><a href="/proexp/show">Professional Experience</a></li>
<li class="admin-nav"><a href="/otherexp/show">Other Experience</a></li>
<li class="admin-nav"><a href="/skills/show">Skills</a></li>
<li class="admin-nav"><a href="/education/show">Education</a></li>
<li class="admin-nav"><a href="/certs/show">Certs</a></li>

<br />
<li class="admin-nav">User:</li>
<li class="admin-nav"><a href="/user/profile">Profile</a></li>
<li class="admin-nav"><a href="/user/showUsers">List</a></li>
<li class="admin-nav"><a href="/user/loginAttempts">Login Attempts</a></li>
<li class="admin-nav"><a href="/user/register">Registration</a></li>
<li class="admin-nav"><a href="/login/logout">Logout</a></li>